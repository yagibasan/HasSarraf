...
namespace HasSarraf.BackOffice.Forms
{
    public partial class frmMain : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        {
            ToolTipItem tt = (barDataReceiverStatus.SuperTip.Items[1]) as ToolTipItem;

            if (status == DataReceiveStatus.FirstLoading)
            {
                barDataReceiverStatus.Caption = "Veriler Güncel Değil";
                tt.Text = Settings.Default.TabelaFiyatZamani.ToString("HH:mm:ss");
                this.barDataReceiverStatus.ImageOptions.Image = imageCollection1.Images[2];
            }
            else if (status == DataReceiveStatus.Received)
            {
                barDataReceiverStatus.Caption = "Veriler Güncel";
                tt.Text = Settings.Default.TabelaFiyatZamani.ToString("HH:mm:ss");
        ...
        void CreateRows()
        {
            ...
            foreach (var item in UtilBL.HasConfig.Urunler.Where(p => p.Goster).OrderBy(p => p.Sira))
            {
                ...
                {
                    ucUrunItem urunItem = new ucUrunItem(item);
        ...
        private void barButtonShowProducts_ItemClick(object sender, ItemClickEventArgs e)
        {
            frmUrunHavuzu frm = new frmUrunHavuzu();
            frm.ShowDialog();
        }

        private void barButtonRefreshAll_ItemClick(object sender, ItemClickEventArgs e)
        {
            SetDataReceiveStatus(DataReceiveStatus.FirstLoading);
            CreateRows();
        }
        ...
        private void barBtnMarquee_ItemClick(object sender, ItemClickEventArgs e)
        {
